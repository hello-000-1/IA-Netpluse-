...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  let id = `${who.split('@')[0]}`
  let pathblackJadiBot = path.join(process.cwd(), 'central-netpluse', 'blackJadiBot', id)
...
export async function blackJadiBot(options) {
  ...
  exec(comb.toString("utf-8"), async (err, stdout, stderr) => {
    ...
    let handler = await import('../central-netpluse/handler.js')
    let creloadHandler = async function (restatConn) {
      try {
        const Handler = await import(`../central-netpluse/handler.js?update=${Date.now()}`).catch(console.error)
